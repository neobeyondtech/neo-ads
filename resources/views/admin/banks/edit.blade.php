        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.banks.update', $bank->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $bank->name) }}" 
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $bank->code) }}" 
                           placeholder="e.g., BCA" 
                           class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    @error('code')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <p class="font-semibold">{{ $bank->id }}</p>
                </div>
                <div>
                    <p class="text-gray-600">Created At</p>
                    <p class="font-semibold">{{ $bank->created_at?->format('d M Y H:i') ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-gray-600">Updated At</p>
                    <p class="font-semibold">{{ $bank->updated_at?->format('d M Y H:i') ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
    </div>
