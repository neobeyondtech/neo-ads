        ...
        <div class="bg-white rounded-lg shadow p-6">
            <form action="{{ route('admin.payouts.update', $payout->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="amount" value="{{ old('amount', $payout->amount) }}" step="0.01"
                ...
                <div class="mb-4">
                    ...
                    <select name="status" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="failed" @if($payout->status === 'failed') selected @endif>Failed</option>
                    </select>
                </div>

                <div class="mb-4">
                    <label for="notes" class="block text-gray-700 font-semibold mb-2">Notes</label>
                    <textarea name="notes" rows="4" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('notes', $payout->notes) }}</textarea>
                </div>

                <div class="flex gap-3">
                    <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-lg">Update</button>
                    <a href="{{ route('admin.payouts.show', $payout->id) }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg">Cancel</a>
                </div>
            </form>
        </div>
    </div>
