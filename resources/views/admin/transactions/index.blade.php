...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        @if(auth()->user()->role->canPerform('create', 'transactions'))
            <a href="{{ route('admin.transactions.create') }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">+ Create</a>
        @else
            ...
            <tbody>
                @forelse($transactions as $transaction)
                    <tr class="border-t hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium">{{ $transaction->advertisement->title ?? 'N/A' }}</td>
                        <td class="px-4 py-3">Rp{{ number_format($transaction->amount, 0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-1 rounded text-white text-xs
                                @if($transaction->payment_status === 'paid') bg-green-500
                                @elseif($transaction->payment_status === 'pending') bg-yellow-500
                                @elseif($transaction->payment_status === 'failed') bg-red-500
                                @else bg-gray-500
                                @endif
                            ">
                        ...
                        <td class="px-4 py-3">{{ ucfirst(str_replace('_', ' ', $transaction->payment_method ?? '-')) }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $transaction->payment_date?->format('M d, Y') ?? '-' }}</td>
                        <td class="px-4 py-3">
