                ...
                <div class="mb-4">
                    ...
                    <select name="province_id" id="province_id" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        ...
                        @foreach(\App\Models\MasterProvince::all() as $province)
                            <option value="{{ $province->id }}" @if(old('province_id', $city->province_id) == $province->id) selected @endif>
                                {{ $province->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    ...
                    <p class="font-semibold">{{ $city->id }}</p>
                </div>
                <div>
                    <p class="text-gray-600">Province</p>
                    <p class="font-semibold">{{ $city->province?->name ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-gray-600">Created At</p>
                    <p class="font-semibold">{{ $city->created_at?->format('d M Y H:i') ?? 'N/A' }}</p>
                </div>
                <div>
                    <p class="text-gray-600">Updated At</p>
                    <p class="font-semibold">{{ $city->updated_at?->format('d M Y H:i') ?? 'N/A' }}</p>
                </div>
            </div>
        </div>
    </div>
